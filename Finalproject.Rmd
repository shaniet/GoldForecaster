...
for(i in 1:5){
  ...
  training.i <- folds[[1]]
  testing.i <- setdiff(1:length(gold$PercChangeForc), training.i)
  gold.test <- gold[testing.i,]
  gold.train <- gold[training.i,]

  # Model
  lin.reg <- lm(PercChangeForc~.-Binary.PercChange, data = gold.train)
...
library(class)
set.seed(75849)
knn_ctrl <- trainControl(method = "cv", number = 5) # Five Fold CV
...
knn_optimize <- train(as.factor(Binary.PercChange) ~., data = gold.c, 
                      method = "knn", trControl = knn_ctrl, tuneGrid = k_values)
